...
const EditCar = () => {
    ...
    if (!form) return <main style={{ padding: '40px 50px' }}><p>Loading...</p></main>

    const totalPrice = calcTotalPrice(form)
    const carColor = EXTERIOR_COLORS[form.exterior] || '#555'
        ...
        <main style={{ flexDirection: 'column', padding: '20px 50px' }}>
            ...
            <article>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <footer>
                        <button type="button" onClick={() => navigate(-1)} style={{ background: '#2c2c2c' }}>Cancel</button>
                        <button type="submit" disabled={loading}>
                            {loading ? 'Saving...' : `Save Changes — ${formatPrice(totalPrice)}`}
                        </button>
                    </footer>
                </form>
            </article>
